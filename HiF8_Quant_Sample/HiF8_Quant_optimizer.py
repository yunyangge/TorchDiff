...
def _sparsity(tensor):
    total_ele = tensor.numel()
    zero_ele = (tensor == 0).sum().item()
    return zero_ele / total_ele

class MomentumQuantizer:
    def __init__(self, optimizer=None):
        self.optimizer = optimizer
        self.qtype = QType("hif8")
        self.scale_max_exp_avg = 512.0
        self.scale_max_exp_avg_sq = None
        self.group_size = 512

    def _quantize(self, tensor, scale_max=None,scale=None):
        """统一量化函数，返回量化后的值和使用的scale""" 
        if scale is not None:     
            scaled = tensor.float() * scale
            quantized = quant_dequant_float(scaled, self.qtype, force_py=False, force_fp32=True)
        ...
        remainder = num_elements % self.group_size
        if remainder != 0:
            pad_size = self.group_size - remainder
            flattened_padded = torch.cat([flattened, torch.zeros(pad_size, dtype=flattened.dtype, device=flattened.device)])
        else:
            flattened_padded = flattened
            pad_size = 0

        num_groups = flattened_padded.numel() // self.group_size
        grouped_tensor = flattened_padded.reshape(num_groups, self.group_size)

        abs_grouped = grouped_tensor.abs()
        max_vals = abs_grouped.max(dim=1).values
        min_vals = torch.where(abs_grouped > 0, abs_grouped, torch.inf).min(dim=1).values

        zero_mask = min_vals == torch.inf
        min_vals = torch.where(zero_mask, torch.tensor(1.0, device=tensor.device), min_vals)

        if scale_max is None:
            scale_from_min = 2 ** (-15) / min_vals
            scale_from_max = 2 ** (15) / max_vals

            scales = torch.minimum(scale_from_min, scale_from_max)
        else:
            scales = torch.tensor(scale_max, device=tensor.device) / (max_vals + 1e-15)

        scales = torch.where(zero_mask, torch.tensor(1.0, device=tensor.device), scales)
        scales_expanded = scales.unsqueeze(1).expand(-1, self.group_size)

        scaled_groups = grouped_tensor.float() * scales_expanded
        quantized_tensor = quant_dequant_float(scaled_groups, self.qtype, force_py=False, force_fp32=True)
        dequantized_groups = quantized_tensor / scales_expanded

        dequantized_padded = dequantized_groups.reshape(-1)
        if pad_size > 0:
            dequantized_flattened = dequantized_padded[:-pad_size]
        else:
            dequantized_flattened = dequantized_padded
        
        quantized_tensor = dequantized_flattened.reshape(original_shape).to(tensor.dtype)
        scale_info = {
            'scales': scales,
        ...
        return quantized_tensor, scale_info

    def _dequantize(self, tensor, scale):
        """反量化函数"""
        print("dequantize scale = ",scale)
        if scale is not None and scale.item() != 1.0:
            return tensor.float() / scale
        return tensor

    def quantize_all_momenta(self):
        """量化所有动量（用于优化器step前）"""
        ...
        for group in self.optimizer.param_groups:
            for p in group['params']:
                ...
                self._quantize_momentum_tensor(state, self.scale_max_exp_avg, 'exp_avg', 'scale_exp_avg')
                self._quantize_momentum_tensor(state, self.scale_max_exp_avg_sq, 'exp_avg_sq', 'scale_exp_avg_sq')

    def _quantize_momentum_tensor(self, state, scale_max, tensor_key, scale_key):
        """量化单个动量张量"""
        if tensor_key in state and state[tensor_key] is not None:
            tensor = state[tensor_key]
            quantized_tensor, scale = self._quantize(tensor, scale_max=scale_max)
            state[tensor_key] = quantized_tensor
            state[scale_key] = scale
            iter_num = os.environ.get('iter', 'default')
            # print(f"maximum value in {tensor_key} in iter {iter_num} = {tensor.max().item()}")
            # print(f"minimum value in {scale_key} in iter {iter_num} = {state[scale_key].min().item()}")
            # if scale_max == 512.0:
            #     print(f"first order: maximum value in quantized in iter {iter_num} = {quantized_tensor.max().item()}")
            #     print(f"first order: scale in iter {iter_num} = {scale.item()}")
            # else:
            #     print(f"second order: maximum value in quantized in iter {iter_num} = {quantized_tensor.max().item()}")
            #     print(f"second order: scale in iter {iter_num} = {scale.item()}")
            # # 记录量化状态
            # self.quantized_tensors[id(tensor)] = (tensor_key, scale_key)

    def dequantize_for_computation(self, tensors, scales):
        """为计算准备的反量化"""
        dequantized_tensors = []
        for tensor, scale in zip(tensors, scales):
            dequantized_tensors.append(self._dequantize(tensor, scale))
        return dequantized_tensors

    def requantize_after_computation(self, dequantized_tensors, scale_max, original_tensors, original_scales):
        """计算后的重新量化"""
        for _, (dequant_tensor, orig_tensor, orig_scale) in enumerate(zip(dequantized_tensors, original_tensors, original_scales)):
            quantized_tensor, scale = self._quantize(dequant_tensor, scale_max=scale_max)
            orig_tensor.copy_(quantized_tensor)
            # orig_scale.copy_(scale['scales'])
...
class AdamW(Optimizer):
    ...
    def __setstate__(self, state):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if "scale_exp_avg" not in p_state:
                    p_state["scale_exp_avg"] = torch.tensor(1.0, device=p.device)
                if "scale_exp_avg_sq" not in p_state:
                    p_state["scale_exp_avg_sq"] = torch.tensor(1.0, device=p.device)
    ...
    def _init_group(
        ...
        for p in group["params"]:
            ...
            if len(state) == 0:
                ...
                # Quantization scales for the momentums
    ...
    def step(self, closure=None):
        ...
        momentum_quantizer.quantize_all_momenta()
...
def _fused_adamw(
    ...
    *,
    ...
    # Dequant using MomentumQuantizer
    quantizer = MomentumQuantizer()
    #dequantized_exp_avgs = quantizer.dequantize_for_computation(exp_avgs, scale_exp_avgs)
    #dequantized_exp_avg_sqs = quantizer.dequantize_for_computation(exp_avg_sqs, scale_exp_avg_sqs)

    dequantized_exp_avgs = exp_avgs
    dequantized_exp_avg_sqs = exp_avg_sqs

    '''print("dequantized_exp_avgs = ",dequantized_exp_avgs)
    print("dequantized_exp_avg_sqs = ",dequantized_exp_avg_sqs)
    print("dequantized_exp_avgs len = ",len(dequantized_exp_avgs))
    print("dequantized_exp_avg_sqs len = ",len(dequantized_exp_avg_sqs))'''


    '''for index, tensor in enumerate(dequantized_exp_avgs):
        dump(tensor, f"exp_avg_index{index}")
    for index, tensor in enumerate(dequantized_exp_avg_sqs):
        dump(tensor, f"exp_avg_sq_index{index}")'''

    iter_num = os.environ.get('iter', 'default')

    '''save_steps = 5
    if int(iter_num) > save_steps: # save dumped files in last 5 steps
        temp_dir = f"/mnt/hdfs/fzy/wxk/optimizer_svg_check_2/step{str(int(iter_num)-save_steps)}/"
        try:
    ...
    for tensor in dequantized_exp_avgs:
        if torch.isnan(tensor).any():
            raise AssertionError(f"dequantized_exp_avgs is nan at iter {iter_num}")
        if torch.isinf(tensor).any():
            raise AssertionError(f"dequantized_exp_avgs is inf at iter {iter_num}")
  
    for tensor in dequantized_exp_avg_sqs:
        if torch.isnan(tensor).any():
            raise AssertionError(f"dequantized_exp_avg_sqs is nan at iter {iter_num}")
        if torch.isinf(tensor).any():
            raise AssertionError(f"dequantized_exp_avg_sqs is inf at iter {iter_num}")
    ...
    grouped_tensors = Optimizer._group_tensors_by_device_and_dtype(
        [params, grads, dequantized_exp_avgs, dequantized_exp_avg_sqs, max_exp_avg_sqs, state_steps]  # type: ignore[list-item]
    )
    for (device, _), (
        (
            device_params_,
    ...
    # Quant using MomentumQuantizer
    quantizer.requantize_after_computation(dequantized_exp_avgs, quantizer.scale_max_exp_avg, exp_avgs, scale_exp_avgs)
    quantizer.requantize_after_computation(dequantized_exp_avg_sqs, quantizer.scale_max_exp_avg_sq, exp_avg_sqs, scale_exp_avg_sqs)
